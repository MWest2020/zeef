...
[Architectuur](../explanation/architectuur.md)). Eén `.eml` kan meerdere documenten opleveren
...
vastgelegd als getypeerde `Relation`s met *evidence*. De near-duplicate-drempel (de cosinus-grens
...
[Scope-filter](scope-filter.md) voor de stage in detail.
...
[profiel](../explanation/architectuur.md#profielen).
...
- **`audit.jsonl`** — de volledige [audit-trail](audit-trail.md).
